...
class RNN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        output, hidden = self.rnn(inputs)   # output is output for every timestep, hidden is only the last one

        # Obtain output layer representations
        outputRep = self.W(output.sum(dim=0))  # get final hidden state

        # Obtain probability dist.
        predicted_vector = self.softmax(outputRep)
